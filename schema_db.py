...
import ctypes
import struct
import head_db  # it is the main memory structure for the table schema
...
class Schema(object):
    ...
    def __init__(self):
        ...
        bufLen = META_HEAD_SIZE + TABLE_NAME_HEAD_SIZE + MAX_FIELD_SECTION_SIZE  # the length of metaHead, table name entries and filedName sections
        ...
        if len(buf) == 0:  # for the first time, there is nothing in the schema file
            ...
            if not isStored:  # only the meta head exists, but there is no table information in the schema file
                ...
                for i in range(tempTableNum):
                    ...
                    # the following is to fetch field information from the body section, and each field is (fieldName, fieldType, fieldLength)
    ...
    # -------------------------------
    # to determine whether the table named table_name exists, depending on the main memory structures
    # input
    #       table_name
    # output
    #       true or false
    ...
    def delete_table_schema(self, table_name):
        ...
        if tmpIndex >= 0:
            ...
            if len(self.headObj.tableNames) > 0:  # there is at least one table after the deletion
                ...
                self.headObj.offsetOfBody = self.headObj.tableNames[-1][2] + self.headObj.tableNames[-1][
                    1] * MAX_FIELD_LEN
                self.WriteBuff()
