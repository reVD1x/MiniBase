    ...
    def insert_index_entry(self, field_value, block_id, offset):
        ...
        if len(field_value.strip()) > 0 and block_id > 0 and offset > 0:  # the following is to insert an index entry into the index file
            if len(self.first_block_buf.strip()) == 0:  # there is no data in the index file
                ...
                struct.pack_into('!i', first_index_block, common_db.BLOCK_SIZE - struct.calcsize('!i'),
                ...
                struct.pack_into('!i?ii', self.meta_index_block, 0, 0, True, 1,
                                 1)  # block_id, has_root, number of levels, root_node_ptr(block_id)
                ...
                temp_block_id, self.has_root, self.num_of_levels, self.root_node_ptr = struct.unpack_from('!i?ii',
                ...
                if self.has_root == True and self.num_of_levels > 0 and self.root_node_ptr > 0:
                    ...
                    while temp_count < self.num_of_levels - 1:  # to search through the internal nodes
                        ...
                        current_node_type, current_num_of_keys = struct.unpack_from('!ii', current_index_block,
                                                                                    struct.calcsize('!i'))
                        ...
                        for i in range(current_num_of_keys):
                            current_key, current_ptr = struct.unpack_from('!10si', current_index_block,
                                                                          struct.calcsize('!iii') + i * (10 + 4))
                    ...
                    if current_node_type == LEAF_NODE_TYPE:  # it is a leaf node
                        if current_num_of_keys < MAX_NUM_OF_KEYS:  # insert the value into the leaf node
                            ...
                            ptr_list = []
                            for i in range(current_num_of_keys):
                                current_key, block_ptr, current_offset = struct.unpack_from('!10sii',
                                                                                            current_index_block,
                                                                                            struct.calcsize(
                                                                                                '!iii') + i * LEN_OF_LEAF_NODE)
                            ...
                            for i in range(len(key_list)):
                                ...
                                struct.pack_into('!10sii', current_index_block,
                                                 struct.calcsize('!iii') + i * LEN_OF_LEAF_NODE, current_key,
                                                 current_id, current_offset)
